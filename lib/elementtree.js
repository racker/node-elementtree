  ...
  if (options.method === "text") {
    ...
    if (options.method === "xml") {
      _serialize_xml(function(data) {
        sb.push(data);
      }, this._root, options.encoding, qnames, namespaces, indent, indent_string, options.lexicalOrder);
    }
    else {
      /* TODO: html */
...
function _serialize_xml(write, elem, encoding, qnames, namespaces, indent, indent_string, lexicalOrder) {
  var tag = elem.tag;
  var text = elem.text;
  var items;
  var i;
  ...
  if (tag === Comment) {
    ...
    if (tag === undefined) {
      ...
      elem.iter(function(e) {
        _serialize_xml(write, e, encoding, qnames, null, newlines ? indent + 1 : false, indent_string, lexicalOrder);
      });
    }
    else {
      write("<" + tag);
      ...
      if (items || namespaces) {
        if (lexicalOrder === undefined || lexicalOrder) {
