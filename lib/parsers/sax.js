...
XMLParser.prototype._handleOpenTag = function(tag) {
  var name = clarkEncodeTag(tag),
      attributes = Object.keys(tag.attributes)
        .reduce(function(d, k) {
          var attr = tag.attributes[k];
          d[clarkEncodeAttribute(attr)] = attr.value;
          return d;
        }, {});
...
XMLParser.prototype._handleCloseTag = function(tag) {
  ...
  this.target.end(clarkEncodeTag(this.parser.tag));
};
...
function clarkEncodeTag(node) {
  return (node.local && node.uri) ? sprintf('{%s}%s', node.uri, node.local) : node.name;
}

function clarkEncodeAttribute(node) {
  return (node.local && node.uri && node.prefix) ? sprintf('{%s}%s', node.uri, node.local) : node.name;
}
